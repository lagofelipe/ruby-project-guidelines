class FrequentFlyer < ActiveRecord::Base
    ...
    has_many :airports, through: :trips
      ...
      def self.medallion_menu
        system "clear"
        puts $pastel1.red("Medallion Menu - Agent: Please select criteria for Medallion Lookup Imformation")
       ...
       def self.medallion_by_city
            system "clear"
            puts $pastel1.red("Medallion Search By Station")
        ...
        def self.medallion_by_city_helper(station_input)
            ...
            name_out = target.map {|flyer|flyer.name}
            puts ""
            puts $pastel1.red("==== Agent: Your results are the following: ========") 
            puts ""
            puts name_out
            puts ""
            puts $pastel1.red("No other Medallion holders assigned to Station or FBO load")
            main_menu
        end
       ...
       def self.finder_by_name
           system "clear"
           puts $pastel1.yellow("Medallion Search By Name")
       ...
       def self.medallion_by_name_helper(name_input)
        ...
        puts $pastel1.red("==== Agent: Your results are the following: ========") 
        puts ""
        puts $pastel1.green("Medallion Holder Information: #{member_out}")
        puts ""
        puts $pastel1.red("No other Medallion holders or companions listed under Members ")
         main_menu
        end
       ...
       def self.medallion_search
            system "clear"
            puts $pastel1.yellow("Medallion Search By Medalion Tier")
        ...
        def self.medallion_search_helper(brass_input)
            ...
            puts med_out
            puts ""
            puts $pastel1.red("No other Medallion holders for Tier ")
            main_menu
        end
